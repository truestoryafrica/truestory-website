...
export async function addLocalStory(input) {
  ...
  const story = {
    ...
    updatedAt: now,
    seoTitle: input.seoTitle || `${input.title} | TrueStory Africa`,
    seoDescription: input.seoDescription || input.excerpt
  };

  await sql`
    insert into stories (id, type, title, slug, category, location, excerpt, body_html, author, reading_time, tags, status, image, alt, date, updated_at, seo_title, seo_description)
    values (${story.id}, ${story.type}, ${story.title}, ${story.slug}, ${story.category}, ${story.location}, ${story.excerpt}, ${story.bodyHtml}, ${story.author}, ${story.readingTime}, ${story.tags}, ${story.status}, ${story.image}, ${story.alt}, ${story.date}, ${story.updatedAt}, ${story.seoTitle}, ${story.seoDescription})

...
import { eventGalleries as fallbackEvents, insights as fallbackInsights, partners as fallbackPartners, services as fallbackServices, site as fallbackSite, stories as fallbackStories, team as fallbackTeam, testimonials as fallbackTestimonials } from "@/content/site";
...
export const getSiteContent = cache(async function getSiteContent() {
  ...
  // erroring out.
  const [localStories, localInsights, localEvents, localSettings] = await Promise.all([
    getLocalStories(),
    getLocalInsights(),
    getLocalEvents(),
    getLocalSettings()
  ...
  return {
    ...
    events: localEvents.length
      ? [...localEvents, ...fallbackEvents.filter((event) => !localEvents.some((localEvent) => localEvent.slug === event.slug))]
      : fallbackEvents,
    team: fallbackTeam
  };
});
...
// A "published" item stays hidden from the public site until its scheduled
// publishAt time passes, letting editors queue up content ahead of time.
